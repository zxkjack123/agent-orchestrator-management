...
- role and class management (`roles_classes.go`): list/create/update/delete roles in `agents.yaml`; list/get/set/delete class templates in `.aom/templates/profiles/`; read the embedded `base.md.tmpl` system template; render profile previews (Zone A + Zone B)
- agent profile generation (`agent_profiles.go`): composes Zone A (system template) + Zone B (class template) + Zone C (custom instructions) into the final `profile.md`
...
- class template writes must use atomic temp-file + `os.Rename` (same directory) to prevent partial writes
...
- REST API handlers for all `/api/v1/` endpoints
- WebSocket bridges (terminal streaming, event log, mailbox)
- SPA static file serving (embedded `web/dist/`)
- Request logging middleware

Sub-packages:

- `handler/` — one file per domain: `projects.go`, `agents.go`, `sessions.go`, `tasks.go`, `status.go`, `extras.go`, `terminal.go`, `fs.go`, `project_actions.go`
- `ws/` — WebSocket terminal bridge (`terminal.go`)

Rules:

- handlers must be thin: validate request, call domain service, write response
- no business logic in handlers — all state transitions live in domain packages
- server package must not be imported by `internal/cli` or domain packages
- WebSocket connections manage their own goroutine lifecycle

## web

Owns:

- React + TypeScript frontend source (`web/src/`)
- Built output embedded into the Go binary (`web/dist/`)

Rules:

- `web/dist/` is generated by `npm run build` and must be committed or built before `go build`
- frontend communicates with the backend via REST API and WebSocket only — no direct domain package imports
- all routes are handled client-side (SPA); the Go server serves `index.html` for any unmatched path
